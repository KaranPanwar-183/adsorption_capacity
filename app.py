...
st.markdown('Enter physicochemical properties for a high-precision adsorption capacity prediction.')
...
st.sidebar.header('2. Elemental & Ratio Composition')
c_cont = st.sidebar.slider('Carbon (C) %', 0.1, 100.0, 82.0)
h_cont = st.sidebar.slider('Hydrogen (H) %', 0.0, 15.0, 1.2)
o_cont = st.sidebar.slider('Oxygen (O) %', 0.0, 60.0, 3.2)
n_cont = st.sidebar.slider('Nitrogen (N) %', 0.0, 30.0, 1.9)
ash = st.sidebar.slider('Ash Content %', 0.0, 90.0, 11.0)

# DIRECT INPUT FOR RATIOS
hc_ratio = st.sidebar.number_input('H/C Ratio', 0.0, 2.0, 0.09)
oc_ratio = st.sidebar.number_input('O/C Ratio', 0.0, 1.0, 0.03)
nc_ratio = st.sidebar.number_input('N/C Ratio', 0.0, 1.0, 0.02)
onc_ratio = st.sidebar.number_input('(O+N)/C Ratio', 0.0, 1.0, 0.06)
...
initial_conc = st.sidebar.number_input('Initial Conc (mg/L)', 0.01, 2000.0, 13.0)
solution_ph = st.sidebar.slider('Solution pH', 0.0, 14.0, 7.0)
adsorp_time = st.sidebar.number_input('Adsorption Time (min)', 1, 10000, 120)
temp = st.sidebar.slider('Adsorption Temp (°C)', 0, 100, 25)
dosage = st.sidebar.number_input('Adsorbent Dosage (g/L)', 0.01, 10.0, 0.08)
rpm = st.sidebar.slider('Stirring Speed (RPM)', 0, 500, 150)
vol = st.sidebar.number_input('Solution Volume (L)', 0.001, 1.0, 0.03)
ion_conc = st.sidebar.number_input('Ion Concentration (mol/L)', 0.0, 1.0, 0.02)
humic = st.sidebar.number_input('Humic Acid (mg/L)', 0.0, 100.0, 0.4)
...
if st.button('Predict Adsorption Capacity'):
    ...
    # Mapping Direct Ratio Inputs
    input_df['H/C'] = hc_ratio
    input_df['O/C'] = oc_ratio
    input_df['N/C'] = nc_ratio
    input_df['(O+N)/C'] = onc_ratio
